    ...
    const onSubmit = async (data) => {
      ...
      try {
        ...
        console.log(campaignData);
          const response = await axiosSecure.patch(`/donationCampaignEdit/${campaign._id}`, campaignData)
    ...
    return (
      <div className='p-8 md:p-6 dark:bg-slate-900 bg-background border border-slate-100 rounded-3xl overflow-hidden shadow-sm max-w-5xl mx-auto'>
        ...
        <div className='border-b border-slate-50 bg-linear-to-r from-rose-50 to bg-transparent p-6 rounded-t-3xl mb-8'>
          <h1 className='text-3xl font-bold flex items-center gap-2 text-primary'> <HandHelping size={32} /> Edit <span className='text-purple-600'>{campaign.petName}</span> Donation Campaign</h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              defaultValue={campaign.requiredAmount}
              type='number'
          ...
          <div className="md:col-span-2 pt-4">
            <button
              type="submit"
              className="w-full py-4 bg-primary text-primary-foreground rounded-2xl font-bold flex items-center justify-center gap-2 hover:shadow-lg transition-all cursor-pointer"
            >
              {
                loading ? <Loader2 className="animate-spin" /> : <span className='flex items-center justify-center gap-1'><Send size={18} /> Update campaign </span>
              }
            </button>
          </div>
        </form>
